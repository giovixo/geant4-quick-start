...
// Photosensors
    ...
    for(G4int i=0; i<100; i++) {
        for (G4int j=0; j<100; j++){
            G4VPhysicalVolume *physDetector = new G4PVPlacement(0, G4ThreeVector(-0.5*m+(i+0.5)*cm, -0.5*m+(j+0.5)*cm, 0.49*m), logicDetector, "physDetector", logicWorld, false, i+j*100, true);
        }
    }
